...
// Same as url, but TLD is optional
    ...
    // Rule 1: Validate MIME types / extensions for ALL selected files
    $.validator.addMethod("allowedFiles", function (value, element) {
        if (this.optional(element) || !element.files || element.files.length === 0) {
            return true;
        }
        ...
        for (let i = 0; i < element.files.length; i++) {
            ...
            // Validate both file.type and extension fallback
            const isValidMime = allowedMimeTypes.includes(file.type);
            const isValidExt = allowedExtensions.includes(fileExtension);

            if (!isValidMime && !isValidExt) {
                return false; // Fail instantly if any single file doesn't match
            }
        }
        return true;
    }, "Only JPG, JPEG, PNG, and PDF files are allowed.");

    // Rule 2: Restrict maximum number of selected files (Max 10)
    ...
    }, $.validator.format("You can upload a maximum of {0} files."));

    // Rule 3 (Optional): Restrict maximum size per file (e.g., 5MB per file)
    $.validator.addMethod("maxFileSize", function (value, element, paramBytes) {
        if (this.optional(element) || !element.files) {
            return true;
        }
        for (let i = 0; i < element.files.length; i++) {
            if (element.files[i].size > paramBytes) {
                return false;
            }
        }
        return true;
    }, "One or more files exceed the maximum allowed size.");// Rule 1: Validate MIME types / extensions for ALL selected files
    return $;
}));
