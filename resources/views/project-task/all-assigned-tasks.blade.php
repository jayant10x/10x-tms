@extends('layouts.vertical', ['title' => 'All Assigned Tasks','subTitle' => 'All Assigned Tasks'])
...
@section('content')
    <div class="card">
        ...
        <div class="card-body p-0">
            @if(!empty($all_assigned_by_me) && count($all_assigned_by_me) > 0)
                <div class="table-responsive">
                    <table class="table align-middle text-nowrap table-hover table-centered mb-0">
                        ...
                        <tbody>
                        @foreach($all_assigned_by_me as $task)
                            <tr>
        ...
        @if(!empty($all_assigned_by_me) && count($all_assigned_by_me) > 0)
            <div class="card-footer">
                <div>
                    Showing
                    {{ $all_assigned_by_me->firstItem() }}
                    to
                    {{ $all_assigned_by_me->lastItem() }}
                    of
                    {{ $all_assigned_by_me->total() }}
                    tasks
                </div>

                <div>
                    {{ $all_assigned_by_me->links() }}
                </div>
            </div>
        @endif
    </div>
