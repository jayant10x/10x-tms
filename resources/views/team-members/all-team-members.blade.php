                            ...
                            @if(!empty($member_val['emp_photo']))
                                <a href="{{ $emp_photo }}" target="_blank"
                                   rel="noopener noreferrer">
                                    @endif
                                    <img src="{{$emp_photo}}" class="avatar-lg rounded-3 border border-light border-3">
                                    <div class="d-block">
                                        <a href="#!"
                                           class="text-dark fw-medium fs-16">{{$member_val['emp_full_name']}}</a>
                                        <p class="mb-0">{{$member_val['emp_email']}}</p>
                                        <div class="mt-1"> <span class="mb-0 text-primary"># {{$member_val['emp_internal_id']}}</span> <b> | </b><span class="badge badge-soft-primary badge-outline-primary rounded-pill me-1 fs-6">{{\App\Enums\DesignationEnum::from($member_val['emp_designation'])->label()}}</span></div>
